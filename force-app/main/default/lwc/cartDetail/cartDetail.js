...
import updateCartItemQuantity from '@salesforce/apex/BeerController.updateCartItemQuantity'
...
export default class CartDetail extends NavigationMixin(LightningElement) {
    ...
    handleSaveAddress() {
        ...
        .then(result => {
            if ( this.addressess ){
                console.log('result Address are avalible ', result);
    ...
    handleQuantityChange(event) {
        ...
        console.log(event);
        console.log('Cart Item Id: '+event.detail.id);
        console.log('Cart Item Qty: '+event.detail.qty);
        const selectedItemQty = event.detail.qty;
        updateCartItemQuantity({cartItemId: selectedItemId, quantity: selectedItemQty}).then(result => {
            // const selectedItem = this.Items.find(
            //     item => item.Id === selectedItemId
            // );
            // selectedItem = result;
            this.cartItemsUpdate(0);
    ...
    cartItemsUpdate(totalAmt){
        ...
        .then(result => {
            ...
            for ( let i =0 ; i < this.Items.length; i ++) {
                if ( this.Items[i]){
                    if(i == 0)
                        this.totalAmount = totalAmt + this.Items[i].Total_Amount__c;
                    else
                    this.totalAmount = this.totalAmount + this.Items[i].Total_Amount__c; 
                }
            }
